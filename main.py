...
def _creer_utilisateurs_demo():
    ...
    db = SessionLocal()
    try:
        # Supprimer les comptes démo superflus tout en conservant 'recruteur' et 'admin'
        db.query(User).filter(~User.username.in_(["recruteur", "admin"])).delete(synchronize_session=False)
        db.commit()
        ...
        # S'assurer que le compte Administrateur 'admin' existe avec le mot de passe 'admin123'
        admin_user = db.query(User).filter(User.username == "admin").first()
        if not admin_user:
            admin_user = User(username="admin", hashed_password=hash_password("admin123"), role="admin", is_active=True)
            db.add(admin_user)
            db.commit()
            logger.info("[Init] Compte Administrateur 'admin' créé avec succès.")
        else:
            # S'assurer qu'il a bien le rôle admin
            if admin_user.role != "admin":
                admin_user.role = "admin"
                db.commit()
            logger.info("[Init] Compte Administrateur 'admin' actif.")
    except Exception as e:
        logger.error(f"[Init] Erreur initialisation comptes RH/Admin : {e}")
        db.rollback()
    finally:
        db.close()
