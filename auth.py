...
def login(user_in: UserLogin, db: Session = Depends(get_db)):
    """Connexion et génération d'un token JWT avec auto-seeding de secours pour Vercel."""
    user = db.query(User).filter(User.username == user_in.username).first()

    # Secours Vercel Serverless (si la base SQLite /tmp a été recréée sans les comptes)
    if not user and user_in.username in ["admin", "recruteur"]:
        try:
            from backend.main import _creer_utilisateurs_demo
